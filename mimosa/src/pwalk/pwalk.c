...
process_arglist(int argc, char *argv[])
{
   ...
   for (narg=1; narg < argc; narg++) {
      ...
      } else if (strncmp(arg, "-dest=", strlen("-dest=")) == 0) {
	 DEST_ARG = malloc(strlen(arg));
	 strcpy(DEST_ARG, arg+strlen("-dest=")); // MIMOSA tree output root dir
